...
class Blackjack
{
    ...
    {
        echo "딜러의 카드: " . $this->getCardString($this->dealerHand[0]) . "\n";
    ...
    while (!$game->isGameOver()) {
        ...
        echo "Hit(1) or Stand(2)? ";
        $input = intval(trim(fgets(STDIN)));
        if ($input == 1) {
            $game->hit();
            echo "Your cards: " . $game->getPlayerHandString() . "\n";
        } elseif ($input == 2) {
            $game->stand();
            break;
        }
    ...
    echo "Dealer's cards: " . $game->getDealerHandString() . "\n";
